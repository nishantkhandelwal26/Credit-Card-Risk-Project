...
with tab1:
    ...
    with tab3:
        ...
        with col1:
            ...
            employment_duration = st.selectbox("Employment Duration", sorted(df["employment_duration"].dropna().astype(str).unique()))
            installment_rate = st.number_input("Installment Rate", min_value=1, max_value=10, value=2)
            personal_status_sex = st.selectbox("Personal Status / Sex", sorted(df["personal_status_sex"].dropna().astype(str).unique()))
            other_debtors = st.selectbox("Other Debtors", sorted(df["other_debtors"].dropna().astype(str).unique()))

        with col2:
            present_residence = st.number_input("Present Residence", min_value=1, max_value=10, value=2)
            property_type = st.selectbox("Property", sorted(df["property"].dropna().astype(str).unique()))
            age = st.number_input("Age", min_value=18, max_value=100, value=30)
            other_installment_plans = st.selectbox("Other Installment Plans", sorted(df["other_installment_plans"].dropna().astype(str).unique()))
            ...
            people_liable = st.number_input("People Liable", min_value=1, max_value=10, value=1)
            telephone = st.selectbox("Telephone", sorted(df["telephone"].dropna().astype(str).unique()))
            foreign_worker = st.selectbox("Foreign Worker", sorted(df["foreign_worker"].dropna().astype(str).unique()))
            ...
            if st.button("Predict Risk"):
                ...
                if pred == 1:
                    st.error(f"Predicted: Bad Risk")
                else:
                    st.success(f"Predicted: Good Risk")
